  ...
  <div>
    <v-app>
      <v-app-bar app color="blue">
        ...
        <v-spacer></v-spacer>
        <v-toolbar-title>Spices</v-toolbar-title>
        <v-spacer></v-spacer>
        <v-icon @click="deleteItem">mdi-trash-can-outline</v-icon>
      </v-app-bar>
      <v-main>
        <v-card v-for="spice in spices"
            :key="spice.id" width="100%" outlined class="mt-6 mb-3">
          <v-list-item
          >
            <v-list-item-content>
              <div class="overline mb-4">SPICE</div>
          ...
          <v-card-actions>
            <v-btn @click="showIngredients(spice.id)" outlined rounded text> See Ingredient </v-btn>
          </v-card-actions>
        </v-card>
      </v-main>
    </v-app>
