  ...
  <div>
    <v-app>
      <v-app-bar app color="blue">
        ...
        <v-spacer></v-spacer>
        <v-toolbar-title>Grains</v-toolbar-title>
        <v-spacer></v-spacer>
        <v-icon @click="deleteItem">mdi-trash-can-outline</v-icon>
      </v-app-bar>
      <v-main>
        <v-card v-for="grain in grains"
            :key="grain.id" width="100%" outlined class="mt-6 mb-3">
          <v-list-item
          >
            <v-list-item-content>
              <div class="overline mb-4">GRAIN</div>
          ...
          <v-card-actions>
            <v-btn @click="showIngredients(grain.id)" outlined rounded text> See Ingredient </v-btn>
          </v-card-actions>
        </v-card>
      </v-main>
    </v-app>
